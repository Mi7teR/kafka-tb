...
**Отправка шардирована по ключу порядка (Task 18).** TigerBeetle не гарантирует порядок между параллельными запросами, а порядок внутри партиции нам нужен. Поэтому батчер держит `batcher.shards` воркеров — по одному на шард, с одной очередью на каждый, — у каждого не больше одного батча in-flight, и команда попадает к воркеру по хешу своего ключа (`topic/partition` у синка, пустой ключ у API — round-robin). Маршрутизация идёт по одному лишь ключу, тип операции в неё не входит: шард владеет ключом целиком, поэтому `create_accounts` и `create_transfers` одной партиции не расходятся по разным воркерам и не летят одновременно. Один ключ → один воркер → две команды одного ключа никогда не в полёте одновременно, то есть порядок применения внутри партиции — это порядок офсетов. Один запрос к TigerBeetle несёт события одного типа, поэтому воркер копит подряд идущие команды одной операции и отправляет накопленное, когда операция сменилась, набрался `max_batch_size` или истёк `linger` — что раньше. Между разными ключами порядок не гарантируется и не требуется. До Task 18 воркер был один на тип операции, и всё стояло в очереди за ним.
