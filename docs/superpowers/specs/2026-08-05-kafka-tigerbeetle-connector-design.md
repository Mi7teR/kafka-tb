...
**Отправка шардирована по ключу порядка (Task 18).** TigerBeetle не гарантирует порядок между параллельными запросами, а порядок внутри партиции нам нужен. Поэтому батчер держит `batcher.shards` воркеров на каждый тип операции, у каждого не больше одного батча in-flight, и команда попадает к воркеру по хешу своего ключа (`topic/partition` у синка, пустой ключ у API — round-robin). Один ключ → один воркер → две команды одного ключа никогда не в полёте одновременно, то есть порядок применения внутри партиции — это порядок офсетов. Между разными ключами порядок не гарантируется и не требуется. До Task 18 воркер был один на тип операции, и всё стояло в очереди за ним.
