...
YAML (viper) с переопределением через переменные окружения `KAFKATB_*`, которые в свою очередь перекрываются флагами командной строки. Конфиг валидируется на старте — сервис падает сразу, а не на первом сообщении, включая неизвестные ключи (в том числе вложенные) в файле. Какие компоненты запускать выбирает не конфиг, а подкоманда cobra: `kafkatb sink`, `kafkatb cdc`, `kafkatb run`.
